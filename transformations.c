...
/*   Updated: 2025/03/12 11:57:44 by amema            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	resize_sphere(t_sphere *sp, float delta_diameter)
{
	...
	new_diameter = (sp->radius * 2.0f) + delta_diameter;
...
void	resize_cylinder_diameter(t_cylinder *cy, float delta_diameter)
{
	...
	new_diameter = (cy->radius * 2.0f) + delta_diameter;
	if (new_diameter < 0.1f)
		new_diameter = 0.1f;
...
void	resize_cylinder_height(t_cylinder *cy, float delta_height)
{
	...
	new_height = cy->height + delta_height;
	if (new_height < 0.1f)
		new_height = 0.1f;
	cy->height = new_height;
	center = scale(0.5f, add(cy->a, cy->b));
	axis = norm(sub(cy->b, cy->a), 1.0f);
